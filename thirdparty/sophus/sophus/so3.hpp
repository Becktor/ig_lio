...
template <class Scalar_, int Options_>
struct traits<Map<Sophus::SO3<Scalar_>, Options_>> : traits<Sophus::SO3<Scalar_, Options_>> {
    static constexpr int Options = Options_;
    using Scalar = Scalar_;
...
template <class Scalar_, int Options_>
struct traits<Map<Sophus::SO3<Scalar_> const, Options_>> : traits<Sophus::SO3<Scalar_, Options_> const> {
    static constexpr int Options = Options_;
    using Scalar = Scalar_;
...
class SO3Base {
   public:
    ...
    /// unit quaternion.
    ///
    SOPHUS_FUNC Sophus::Vector<Scalar, num_parameters> params() const { return unit_quaternion().coeffs(); }
    ...
    SOPHUS_FUNC TangentAndTheta logAndTheta() const {
        ...
        if (squared_n < Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon()) {
            ...
            SOPHUS_ENSURE(abs(w) >= Constants<Scalar>::epsilon(), "Quaternion (%) should be normalized!",
    ...
    SOPHUS_FUNC void normalize() {
        ...
        SOPHUS_ENSURE(length >= Constants<Scalar>::epsilon(), "Quaternion (%) should not be close to zero!",
    ...
    // newly-added
    // right jacobian
    SOPHUS_FUNC static Transformation jl(const Point& Omega) {
        ...
        return (sin_theta / theta) * Transformation ::Identity() + (1 - sin_theta / theta) * a * a.transpose() +
               (1 - cos_theta) / theta * hat(a);
    }

    SOPHUS_FUNC static Transformation jl_inv(const Point& Omega) {
        Scalar theta = Omega.norm();
        if (theta < 1e-6) {
            return Transformation::Identity();
        }
        ...
        return 0.5 * theta * cot_half_theta * Transformation ::Identity() +
               (1 - 0.5 * theta * cot_half_theta) * a * a.transpose() - 0.5 * theta * hat(a);
    }

    SOPHUS_FUNC static Transformation jr(const Point& Omega) { return jl(-Omega); }
    SOPHUS_FUNC Transformation jr() { return jr(log()); }
    SOPHUS_FUNC Transformation jr_inv() { return jl_inv(-log()); }
    SOPHUS_FUNC static Transformation jr_inv(const Point& Omega) { return jl_inv(-Omega); }
    SOPHUS_FUNC static Transformation jr_inv(const SO3Base<Derived>& R) { return jr_inv(R.log()); }
...
class SO3 : public SO3Base<SO3<Scalar_, Options>> {
   public:
    ...
    ///
    template <class OtherDerived>
    SOPHUS_FUNC SO3(SO3Base<OtherDerived> const& other) : unit_quaternion_(other.unit_quaternion()) {}
    ...
    SOPHUS_FUNC SO3(Transformation const& R) : unit_quaternion_(R) {
        ...
        SOPHUS_ENSURE(R.determinant() > Scalar(0), "det(R) is not positive: %", R.determinant());
    }
    ...
    ///
    template <class D>
    SOPHUS_FUNC explicit SO3(Eigen::QuaternionBase<D> const& quat) : unit_quaternion_(quat) {
    ...
    ///
    SOPHUS_FUNC static SO3<Scalar> expAndTheta(Tangent const& omega, Scalar* theta) {
    ...
    ///
    static SOPHUS_FUNC SO3 rotX(Scalar const& x) { return SO3::exp(Sophus::Vector3<Scalar>(x, Scalar(0), Scalar(0))); }

    /// Construct y-axis rotation.
    ///
    static SOPHUS_FUNC SO3 rotY(Scalar const& y) { return SO3::exp(Sophus::Vector3<Scalar>(Scalar(0), y, Scalar(0))); }

    /// Construct z-axis rotation.
    ///
    static SOPHUS_FUNC SO3 rotZ(Scalar const& z) { return SO3::exp(Sophus::Vector3<Scalar>(Scalar(0), Scalar(0), z)); }
    ...
    static SO3 sampleUniform(UniformRandomBitGenerator& generator) {
        ...
        return SO3(QuaternionMember(a * sin(u2), a * cos(u2), b * sin(u3), b * cos(u3)));
    }
   ...
   protected:
    ...
    ///
    SOPHUS_FUNC QuaternionMember& unit_quaternion_nonconst() { return unit_quaternion_; }
...
/// quaternion).
template <class Scalar_, int Options>
class Map<Sophus::SO3<Scalar_>, Options> : public Sophus::SO3Base<Map<Sophus::SO3<Scalar_>, Options>> {
   public:
    ...
    /// Accessor of unit quaternion.
    ///
    SOPHUS_FUNC Map<Eigen::Quaternion<Scalar>, Options> const& unit_quaternion() const { return unit_quaternion_; }

   protected:
    /// Mutator of unit_quaternion is protected to ensure class invariant.
    ///
    SOPHUS_FUNC Map<Eigen::Quaternion<Scalar>, Options>& unit_quaternion_nonconst() { return unit_quaternion_; }
...
/// quaternion).
template <class Scalar_, int Options>
class Map<Sophus::SO3<Scalar_> const, Options> : public Sophus::SO3Base<Map<Sophus::SO3<Scalar_> const, Options>> {
   public:
