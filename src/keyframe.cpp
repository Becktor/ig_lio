...
private:
  ...
  void DeclareParams(){
    ...
    this->declare_parameter<std::string>("odom/lidar_type", "ouster");
    ...
    this->declare_parameter<std::string>("odom/lidar_frame","ouster");
  ...
  void Topics(){
    ...
    imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
        imu_topic, 10, std::bind(&IG_LIO_KEYFRAME_NODE::ImuCallBack, this, _1));
    cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
    "submap_real", 10, std::bind(&IG_LIO_KEYFRAME_NODE::CloudCallBack, this, std::placeholders::_1));



    // Setup publishers
    current_scan_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/submap/current_scan", 10);
    keyframe_scan_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/submap/keyframe_scan", 10);
...
//       sensor_measurement.lidar_end_time_) {
//     LOG(WARNING) << "The time is different" << std::endl;
//     return false;
//   }
  double last_imu_time = imu_buff.back().header.stamp.sec + imu_buff.back().header.stamp.nanosec * 1e-9;
  double time_difference = sensor_measurement.lidar_end_time_ - last_imu_time;

    if (time_difference > 0) {
        LOG(WARNING) << "The IMU is behind the LiDAR by " << time_difference <<"Lidar time is " << sensor_measurement.lidar_end_time_ <<" seconds." << std::endl;
        return false;
    } else if (time_difference < 0) {
        // This case means the IMU is ahead of LiDAR, which might be unusual in typical operation.
        LOG(WARNING) << "The IMU is ahead of the LiDAR by " << -time_difference << " seconds." << std::endl;
        // return false;
    }
...
void Process() {
  ...
  scan_msg.header.frame_id = this->map_frame;
  ...
  if (is_first_keyframe || delta_p.block<3, 1>(0, 3).norm() > 1.0 ||
    ...
    keyframe_scan_msg.header.frame_id = "submap";
  ...
  LidarType lidar_type_ = LidarType::OUSTER;
